...
impl Log {
    ...
    pub fn open(bot_id: usize) {
        ...
        let mut file = File::create(&filename).unwrap_or_else(|_|panic!("Couldn't open file {} for logging!", &filename));
    ...
    fn panic_inner(log: &mut Log, message: &str) -> ! {
        if log.file.is_none() {
            ...
            let file = File::create(&filename).unwrap_or_else(|_|panic!("Couldn't open file {} for logging!", &filename));
            log.file = Some(file);
        }
